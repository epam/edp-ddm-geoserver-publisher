...
import com.epam.digital.data.platform.geoserver.config.properties.SchemaCrawlerProperties;
...
public class SchemaCrawlerConfig {
  ...
  private InclusionRule includeTables(SchemaCrawlerProperties config) {
    return name -> config.getExcludeTableSuffixes().stream().noneMatch(name::endsWith);
  }

  @Bean
  public LoadOptionsBuilder loadOptionsBuilder() {
    return LoadOptionsBuilder.builder().withSchemaInfoLevel(SchemaInfoLevelBuilder.standard());
  }

  @Bean
  public SchemaCrawlerOptions schemaCrawlerOptions(
      LoadOptionsBuilder loadOptionsBuilder, LimitOptionsBuilder limitOptionsBuilder) {
